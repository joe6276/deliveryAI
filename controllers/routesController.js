...
async function getRouteRecommendation(origin, destination) {
  ...
  const routes = await getAvailableRoutes(origin, destination);
  
  const recommendation = await recommendBestRoute(routes, origin, destination);
...
async function getRouteRecommendations(req,res){
    try {
        
        const {origin, destination}= req.body

        const result= await getRouteRecommendation(origin,destination)
